...
<?
	...
	$struct = new Structure(_DIR_STRUCTURES_PATH . "cms_brands.xml");

	$entity = new Entity ($struct, $id, $session);
	$function = $session->getParameter("function");

	$description = $entity->getDescription();
	//echo "<p><b>SÅ‚ownik: ".$dict->getDescription()."</b></p>";
	...
	if ($function == "add") {
		...
		$entity->moveDown($id, $field, "", false);
		$function = "";
	}
	...
	if ($function == "delete.start") {
		...
		$browse->addButton($PHP_SELF."?function=down&id=%id%&offset=".$session->getParameter("offset"), "dol‚", 10,"_down.gif");
