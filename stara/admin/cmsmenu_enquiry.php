...
<?
	...
	$resId = $session->base->dql("SELECT id FROM cms_enquiries WHERE menuId=".$menuId);
	if ($resId) {
		$id = $session->utils->toInt($resId[0][0]);
	}

   	$struct = new Structure(_DIR_STRUCTURES_PATH . "cms_menu_enquiries.xml");

   	$entity = new CmsMenu ($id, $session, $struct);
    $entity->setCmsElementType("ENQUIRY");
