...
class Template {
	...
	function make_filename($filename, $xs_include = false)
	{
		...
		{
			if (substr($this->root, strlen($this->root)-1) == "/")
				return substr($this->root, 0, strlen($this->root)-1) . '/' . $filename;
			else
	...
	function set_filename($handle, $filename, $xs_include = false, $quiet = false)
	{
		...
		//echo $this->files_cache[$handle] . " .. ".$this->files[$handle];
		
		
		// checking if tpl and/or php file exists
		if($this->files_cache[$handle] == "" && !@file_exists($this->files[$handle]))
		{
	...
	function assign_var_from_handle1($varname, $handle)
	{
		ob_start("win2iso");
		//ob_start();
	...
	function assign_vars1($vararray)
	{
		$ret = "";
		foreach($vararray as $key => $val)
		{
	...
	function assign_var1($varname, $varval)
	{
		return $this->vars[$varname] = $varval;
...
function plCharset($string, $type = "win2iso") {
